...
def idx_to_state(env, state):
    ...
    n=len(state)
    res =type(state) is tuple
    if res == True  :
      nstate=state[0]
    else :
      nstate=state

    position_idx = int((nstate[0] - env_low[0]) / env_distance[0])
    velocity_idx = int((nstate[1] - env_low[1]) / env_distance[1])
...
if __name__ == '__main__':
    ...
    aa=os.getcwd()
    print (aa)
    q_table = np.load(file="mountaincar/app/results/app_q_table.npy")
